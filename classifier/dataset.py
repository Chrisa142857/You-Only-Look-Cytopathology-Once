...
class SequenceDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if 'withWH' in self.name and self.top_n != -1 and self.bottom_n != -1:
            if not self.fm_exists:
                print('%s does not exist! Need feature map' % os.path.join(r'D:\WSI_analysis\rnn_input\%s_sequences%s' % (self.data_root, self.name), slide + '.pth'))
                exit()
            data = self.average_sample(slide, data, top_n, bottom_n)
        else:
    ...
    def average_sample(self, slide, data, top_n, bottom_n):
        top_step = int(len(self.maxs[slide]) / top_n) + 1
        bottom_step = int(len(self.mins[slide]) / bottom_n) + 1
        if top_n < 0:
            top_data = data[-1*top_n:]
        else:
            top_data = data[-1*len(self.maxs[slide])::top_step]
        if bottom_n == 0:
            bottom_data = data[:bottom_n]
        else:
            bottom_data = data[:len(self.mins[slide]):bottom_step]
