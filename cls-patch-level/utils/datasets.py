...
def otsu_foreground_pt(img_size, center_box, srp_file, otsu_level=8):
    xmin, ymin, xmax, ymax = center_box
    xmin, ymin, xmax, ymax = int(xmin), int(ymin), int(xmax), int(ymax)
    ...
    xs = [x for x in range(xmin, xmax, img_size[0]) for y in range(ymin, ymax, img_size[1]) if x+img_size[0] <= xmax and y+img_size[1] <= ymax]
    ys = [y for x in range(xmin, xmax, img_size[0]) for y in range(ymin, ymax, img_size[1]) if x+img_size[0] <= xmax and y+img_size[1] <= ymax]
    pts = [[x, y] for x, y, f in zip(xs, ys, fore.reshape(-1)) if f == 255]
    return pts
...
class SrpDataset(Dataset):
    ...
    def otsu_tile(self, img_size):
        ...
        for srp_file, (w, h) in zip(self.srp_files, self.wh_list):
            ...
            end_pt = [w/2 + self.center_side/2, h/2 + self.center_side/2]
            center_box = start_pt + end_pt
            LeftTopPts = otsu_foreground_pt(img_size, center_box, srp_file)
